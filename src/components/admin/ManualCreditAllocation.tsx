    ...
    try {
      ...
      if (data.success) {
        ...
        toast.warning(data.message || 'Could not allocate credits', {
          description: data.currentCredits
            ? `Current credits: ${data.currentCredits}`
            : undefined,
        });
      }
    } catch (error) {
  ...
  return (
    <Card className="border-warning/20">
      <CardHeader>
        ...
        <CardDescription>
          Allocate credits from Stripe or make manual corrections. For standard
          payment recording, use{' '}
          <Link
            to="/admin-dashboard?tab=schedule"
            className="underline text-primary"
          >
            Class Logs → Add Credits
          </Link>
          .
        </CardDescription>
      </CardHeader>
      <CardContent>
        <Tabs defaultValue="allocate" className="w-full">
          <TabsList className="grid w-full grid-cols-2">
            <TabsTrigger value="allocate">Stripe Allocation</TabsTrigger>
            <TabsTrigger value="adjust">Manual Correction</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="allocate" className="space-y-4 mt-4">
            ...
            <div className="text-xs text-muted-foreground space-y-1 pt-2">
              <p>
                • Creates subscription record based on active Stripe
                subscription
              </p>
          ...
          <TabsContent value="adjust" className="space-y-4 mt-4">
            ...
            <div className="text-xs text-muted-foreground space-y-1 pt-2 border-t">
              ...
              <p>
                • Used for compensations, corrections, or special circumstances
              </p>

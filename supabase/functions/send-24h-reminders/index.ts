...
Deno.serve(async (req) => {
  ...
  const rateLimitKey = getRateLimitKey(req, 'send-24h-reminders');
  ...
  try {
    ...
    logStep('Found classes needing 24h reminders', {
    ...
    for (const cls of classes ?? []) {
      ...
      const [tutorRes, studentRes] = await Promise.all([
        supabase
          .from('profiles')
          .select('email, first_name, last_name, notify_class_reminders')
          .eq('id', cls.tutor_id)
          .single(),
        supabase
          .from('profiles')
      ...
      const studentName =
        ...
        student.email;
      const formattedDate = new Date(cls.date + 'T12:00:00').toLocaleDateString(
        'en-US',
        {
          weekday: 'long',
          month: 'long',
      ...
      const buildEmailHtml = (
        ...
        const otherPerson =
          role === 'tutor' ? `Student: ${studentName}` : `Tutor: ${tutorName}`;
        return `
      ...
      if (student.notify_class_reminders !== false) {
        try {
          ...
          logStep('Failed to email student', {
            email: student.email,
            error: String(e),
          });
        }
    ...
    logStep('Completed', {
      classesProcessed: classes?.length ?? 0,
      emailsSent,
    });
    ...
    return new Response(
      JSON.stringify({
        success: true,
        classes_processed: classes?.length ?? 0,

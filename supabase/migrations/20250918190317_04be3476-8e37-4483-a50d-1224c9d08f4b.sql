-- Fix the assignment issue with updated role validation trigger

-- 1) Update the role validation trigger to handle system changes
...
BEGIN
  ...
  IF initiating_user IS NULL THEN
    -- Log the role change with system user
...
-- 3) Sync role-based tables
SELECT public.sync_user_roles();

-- 4) Clean up tutor_student_assigned and convert to profile IDs
